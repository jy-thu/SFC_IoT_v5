function [result_P,Total_benifit,exitflag_P,output_P, Accept_num, c_use_result, g_use_result, choosen_nodes, Accepted_Requests] = Solve_PR_batch_Iterat_1(...
    beta, P_now, P_matrix_new, prob, V, c_now, g_now, Startfrom, Pointto, alpha, requests_K, cost_up, NoUp, HaveUpCost, idxs_d, Allow, K, gamma)
% 基于最短路径的Placement+Routing整体问题求解
% 用于PRBI算法，有迭代

% 1函数：8.10修改, 为每个请求k计算P_k
...
% 生成一组新的P矩阵，每个P矩阵的每一行i，依概率prob(i)选择新/旧P中的对应行
P_k = zeros(V^2, E, K);
for k = 1:K
    P_k(:, :, k) = P_now;
end
for i = 1:V^2
% 对于P矩阵的第i行，在所有K个请求中，共有K*prob(i)个请求使用新路径
    change_id = randperm(K, ceil(K*prob(i)));      % 随机选择一些（总数固定）请求，使用新路由矩阵P
    for nn = 1:size(change_id,2)
        P_k(i, :, change_id(nn)) = P_matrix_new(i, :);
    end
end
...
if exitflag_P == 1
    ...
    % 统计资源占用情况
    % evaluate函数，计算优化表达式在优化结果处的值
    Y_1_result = evaluate(Y_1, result_P);
    Y_d_result = evaluate(Y_d, result_P);

    g_use_result = zeros(E,1);
    c_use_result = zeros(V,1);
    for k = 1:K
        gg_result = Y_1_result(:,:,k)' * requests_K.b_1;      % E*1 vector
        g_use_result = g_use_result + gg_result;
        cc_result = Y_d_result(:,:,k)' * requests_K.c_cost;  % V*1 vector
        c_use_result = c_use_result + cc_result;
    end
